...
1. **Read `plan.md` in full before touching a file** — at minimum the Status block, the
   Decisions log, the Construct API, and this epoch's section. Decisions in the plan are
   ...
   *most* settled part. If you disagree, implement as written and record the disagreement
   in your handoff.
2. **Read every `.claude/rules/*.md` file whose `paths` cover what this epoch touches**
   before planning, not after your first file read.
...
4. **Delegate per the plan's Delegation plan**: `implementer` and `verifier` agents
   (`.claude/agents/`, both sonnet) for chunks that carry a one-line acceptance check;
   keep design, construct-API judgment, every AWS interaction, and every measurement for
   yourself. Verify a subagent's confident claim before building on it.
5. **AWS**: profile `admin`, region `us-east-1` explicit on every call. Never
   `delete-stack`/`destroy` a name you have not just read back from
   `aws cloudformation list-stacks`; the account hosts other production sites. If SSO has
   expired, ask the user to run `! aws sso login --profile admin`.
6. **Every AWS resource this epoch creates needs its teardown proven** before the epoch
   is called done (see the plan's Cost guardrails).
