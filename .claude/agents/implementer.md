...
Before editing, read `CLAUDE.md` and every rule file in `.claude/rules/` whose `paths`
frontmatter covers the files you will touch. They also load on their own when you read
matching files, but reading them first is what keeps you from planning against a wrong
assumption.
...
- Match the surrounding style: no semicolons, single quotes, no formatter.
